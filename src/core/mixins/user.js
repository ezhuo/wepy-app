...
export default class userMixin extends wepy.mixin {
  ...
  $getUserInfo(callback) {
    // 顶级容错
    if (!this.$parent || !this.$parent.globalDataSet) return
    // 取缓存信息
    const user = this.$parent.globalDataSet('user')
    // 不重复获取用户信息
    if (user && user.nickName) {
      this.helpers.isFunction(callback) && callback(user)
      this.$apply()
      return user
    }
    // 首次获取用户信息
  ...
  $login(success = () => {}, noAutoLogin) {
    ...
    wepy.login({
      success: (res) => {
        ...
        if (noAutoLogin) {
          // 串行回调
          this.helpers.isFunction(success) && success(res)
          this.$apply()
          return
        }
